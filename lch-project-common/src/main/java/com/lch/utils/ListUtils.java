 ...
 * 如果不到数据库查询，直接在第一次查出来的list中排序，无疑会提高系统的性能。 下面就写一个通用的方法，对list排序， 至少需要满足以下5点：
 * ①.list元素对象类型任意 ---->使用泛型解决
...
public class ListUtils extends org.apache.commons.collections.ListUtils {
	...
	public static <T> List<T> getRandomList(List<T> list, int size) {
		...
		for (int i = 0; i < size; i++) {
			newList.add(list.remove(new Random().nextInt(list.size())));
		}
		return newList;
	}
	 ...
	 */
	@SafeVarargs
	public static <T> List<T> merging(List<T>... lists) {
		List<T> newList = Lists.newArrayList();
		Stream.of(lists).forEach(list -> {
			if (ListUtils.isNotBlank(list)) {
				newList.addAll(list);
			}
		});
		return new ArrayList<>((Set<T>) Sets.newHashSet(newList));
	}
	...
	public static <T> List<T> strToList(String str) {
		return (List<T>) Splitter.on(",").trimResults().splitToList(str);
	}
